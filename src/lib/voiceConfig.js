...
export function getVoiceId(voiceType) {
  return ELEVENLABS_VOICE_MAP[voiceType] || ELEVENLABS_VOICE_MAP.onyx;
}

/** OpenAI TTS fallback */
async function openAiTTS(text, voiceType, signal) {
...
export async function elevenLabsTTS(text, voiceType, signal) {
...
async function openAiSTT(audioBase64, mimeType, signal) {
...
export async function elevenLabsSTT(audioBase64, mimeType = 'audio/wav', signal) {
  try {
    ...
    console.warn('ElevenLabs STT failed – falling back to OpenAI Whisper:', err.message);
    return openAiSTT(audioBase64, mimeType, signal);
  }
}
