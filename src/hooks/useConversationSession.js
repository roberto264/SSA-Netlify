...
export function useConversationSession({
  ...
  const { data: persona } = usePersonaById(personaId);
  ...
  // ─── TTS with mutex ──────────────────────────────────────────────
  const playTTS = useCallback(async (text, voice) => {
  ...
  const sendTextMessage = useCallback(async (text) => {
    ...
    const currentMessages = [...messagesRef.current, { role: 'user', content: text }];
  ...
  // ─── Save & Navigate ──────────────────────────────────────────────
  const saveAndNavigateInternal = useCallback(async (feedbackData) => {
    setIsSaving(true);
    try {
  ...
  const saveAndNavigate = useCallback(async (feedbackData) => {
