...
export default function EmployeeDetail({ user, onBack }: EmployeeDetailProps) {
  ...
  const softSkillKeys = ['gesprachsfuhrung', 'aktives_zuhoren', 'klarheit', 'einwand_behandlung', 'empathie', 'uberzeugungskraft'] as const;
  ...
  const avgSoftSkills = softSkillKeys.map(key => {
    const values = sessions.filter(s => s[key] != null).map(s => s[key] as number);
    return {
      key,
  ...
  const rollenspielMinutes = sessions.reduce((acc, s) => {
    const dur = (s as any).duration_seconds;
  ...
  // ─── Performance Rating ────────────────────────────────
  let performanceLevel: 'excellent' | 'good' | 'average' | 'needs_improvement' | 'no_data' = 'no_data';
...
import type { RollenspielSession } from '@/types/database';
...
function SessionList({ sessions }: { sessions: RollenspielSession[] }) {
  ...
  return (
    <div className="space-y-2">
      {sorted.map(s => {
        ...
        const skills = (['gesprachsfuhrung', 'aktives_zuhoren', 'klarheit', 'einwand_behandlung', 'empathie', 'uberzeugungskraft'] as const)
          .filter(k => s[k] != null);
