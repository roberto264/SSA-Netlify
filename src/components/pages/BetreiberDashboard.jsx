...
import { Loader2, Search, Trophy, MessageCircle, BarChart3, Target, CreditCard, Users, Building2, Clock, ArrowLeft } from 'lucide-react';
import { useAllUsers, useFirmen } from '../../lib/database';
import { useModules, usePersonas } from '../../hooks/useContent';
...
export function BetreiberDashboard() {
  ...
  // Billing stats
  const privatUsers = users.filter((u) => u.role === 'privat');
  const activeSubs = allFirmen.filter((f) => f.subscription_status === 'active').length;
  const trialingSubs = allFirmen.filter((f) => f.subscription_status === 'trialing').length;
  const totalSeats = allFirmen.reduce((acc, f) => acc + (f.seat_limit || 0), 0);
  ...
  const statsData = [
    ...
    { label: 'Quiz bestanden', value: users.reduce((acc, u) => acc + (u.quiz_ergebnisse?.filter((q) => q.passed).length || 0), 0), icon: MessageCircle, color: 'text-purple-600', bg: 'bg-purple-50' },
  ];
              ...
              {filteredUsers.map((user) => {
                ...
                return (
                  <tr key={user.id} className="hover:bg-muted/30 cursor-pointer transition-colors" onClick={() => setSelectedUser(user)}>
                    ...
                    <td className="px-4 py-3 text-sm text-muted-foreground">{user.quiz_ergebnisse?.filter((q) => q.passed).length || 0}</td>
...
function BillingOverview({ allFirmen, users, privatUsers, activeSubs, trialingSubs, totalSeats }) {
  const statusColors = {
        ...
        <CardContent className="p-0">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-border">
                {allFirmen.map((f) => {
                  const firmaUserCount = users.filter((u) => u.firma === f.name).length;
